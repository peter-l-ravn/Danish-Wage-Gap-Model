...
def plot_model(model_baseline):
    ...
    def mean_over_time(model, variable):
        return np.array([weighted_mean(variable[:, t], model.sol.mass[:, t]) for t in range(variable.shape[1])])
...
def plot_wage_gap(model, young_max, old_min, x_size=8, y_size=5):
    ...
    def weighted_mean(values, mass):
        valid = ~np.isnan(values) & ~np.isnan(mass)
        if not np.any(valid) or np.sum(mass[valid]) == 0:
            return np.nan
        return np.sum(values[valid] * mass[valid]) / np.sum(mass[valid])

    T = model.sol.age.shape[1]
    ...
    plt.plot(wage_gap, linewidth=2)
    plt.title(f"Wage Gap: Old (≥{old_min}) − Young (≤{young_max})")
    plt.xlabel("Time")
...
def plot_mean_age_high_skill(model, x_size=8, y_size=5):
    ...
    for t in range(T):
        high_skill = model.sol.l_h[:, t] == 1
        mean_age[t] = weighted_mean(model.sol.age[high_skill, t], model.sol.mass[high_skill, t])

    plt.figure(figsize=(x_size, y_size))
    plt.plot(mean_age, linewidth=2)
